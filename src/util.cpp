...
const char * const BITCOIN_CONF_FILENAME = "digitaleuro.conf";
const char * const BITCOIN_PID_FILENAME = "digitaleurod.pid";
...
#else
    const char* pszModule = "digitaleuro";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Digitaleuro";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Digitaleuro";
#else
    // Unix
    return pathRet / ".digitaleuro";
#endif
#endif
}
